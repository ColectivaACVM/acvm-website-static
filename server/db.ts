...
// Image storage queries
export async function createImage(image: InsertImage): Promise<Image | null> {
  const db = await getDb();
  if (!db) {
  ...
  try {
    ...
    const [created] = await db.select().from(images).where(eq(images.id, result[0].insertId as any)).limit(1);
    return created || null;
  } catch (error) {
...
export async function getUserImages(userId: number): Promise<Image[]> {
  const db = await getDb();
  if (!db) {
...
export async function deleteImage(imageId: number, userId: number): Promise<boolean> {
  ...
  try {
    const result = await db.delete(images).where(
